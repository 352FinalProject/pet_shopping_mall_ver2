...
<section class="common-section" id="#">
	<div class="common-container">
		<div class="product-div">
			<div class="product-img">
				<img
					src="${pageContext.request.contextPath}/resources/upload/product/${productDetail.thumbnail}"
					width="400px" />
			</div>
			<div class="product-info">
				<div id="product-title">${productDetail.productName}</div>
				<div class="product-assist">
					...
					<div class="product-assist">
						<img
							...
							alt="별점" /> <span>${productReviewStarAvg.reviewStarRate}</span>
						<span>&nbsp;|&nbsp;</span> <span><a
							href="#product-review-box">후기 ${productDetail.reviewCnt}건</a></span>
					</div>
				</div>
				<hr class="hr-line" />
				<!-- 적립금 -->
				...
				<hr class="hr-line" />
				<c:set var="productOptions" value="${productDetail.productOptions}" />
				<c:if test="${fn:length(productOptions) eq 1}">
					...
					<div class="additional-price-container"
						...
						<div class="additional-price"
							id="${productOptions[0].productDetailId}">${productOptions[0].additionalPrice}</div>
					</div>
				</c:if>
				<c:if test="${fn:length(productOptions) gt 1}">
					<div>
						<select name="product-option">
							<c:forEach items="${productOptions}" var="productOptions"
								varStatus="vs">
								<c:if test="${empty productOptions.optionName}">
									<option class="options"
										value="${productOptions.productDetailId}">[옵션 선택 안함]</option>
								</c:if>
								<c:if test="${not empty productOptions.optionName}">
									<option class="options"
										value="${productOptions.productDetailId}">[${productOptions.optionName}]
										${productOptions.optionValue} (+
										${productOptions.additionalPrice})</option>
								</c:if>
							</c:forEach>
						</select>
					</div>
					...
					<div class="additional-price-container"
						id="additional-price-container" style="display: none;">
						<c:forEach items="${productOptions}" var="productDetail"
							varStatus="vs">
							<div class="additional-price"
	...
	<div class="review-div">
		<ul class="review-product-utility">
           ...
           작성된 리뷰가 없습니다. 
			...
			<c:if test="${not empty productDetail.reviews}">
				<div class="review-avg-star">
					<div>${productDetail.reviewStarRateAvg}</div>
					<div class="review-avg-star2">
						<c:choose>
							<c:when test="${productDetail.reviewStarRateAvg >= 4.5}">★★★★★</c:when>
							<c:when test="${productDetail.reviewStarRateAvg >= 3.5}">★★★★☆</c:when>
							<c:when test="${productDetail.reviewStarRateAvg >= 2.5}">★★★☆☆</c:when>
							<c:when test="${productDetail.reviewStarRateAvg >= 1.5}">★★☆☆☆</c:when>
							<c:when test="${productDetail.reviewStarRateAvg >= 0.1}">★☆☆☆☆</c:when>
				...
				<!-- 리뷰 시작 구분선 -->
				<c:forEach items="${productDetail.reviews}" var="review" varStatus="vs">
					<li>
							...
							<ul class="pagination pagination-sm" style="margin-top: 10px">
								<c:forEach begin="1" end="${totalPages}" var="pageNumber">
									<li class="page-item ${page == pageNumber ? 'active' : ''}">
										<a class="page-link"
										href="${pageContext.request.contextPath}/product/productDetail.do?productId=${productDetail.productId}&page=${pageNumber}">
	...
	<div class="product-bottom">
		<div class="product-bottom2">
			<div class="product-bottom3">
				<span id="product-bottom-title" style="font-size: 15px;">${productDetail.productName}</span>
				<br /> <span id="product-price"
					style="font-size: 15px; font-weight: 600;"> <fmt:formatNumber
						value="${productDetail.productPrice}" pattern="#,###" /> 원
				</span>
			</div>
	...
	<form:form id="addCartFrm">
		...
		<input type="hidden" value="${productDetail.productOptions[0].productDetailId}"
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const originalPrice = parseInt('<c:out value="${productDetail.productPrice}"/>');
